...
def get_link_anom_det_data_TRANS_TGB(
    ...
    val_ratio: Optional[float] = 0.10,  # default 0.15
    test_ratio: Optional[float] = 0.80,  # default 0.15
    ...
    print(f"Val ratio: {val_ratio}, Test ratio: {test_ratio}.")
    ...
    if (
        dataset.name == "darpa-cadets-tt-01"
        or "darpa-cadets-tt-01-exec-path"
        or "darpa-cadets-tt-exec-path-01-wo-default"
        or "darpa-cadets-tt-exec-path-01-wo-default-reduced"
        or "darpa-trace"
    ):  # TODO: make it more general
        print(f"No anomalies should be loaded, they are already in the dataset.")
    else:
...
def get_link_anom_det_data_TRANS_TGB_classificaiton(
    dataset_name: str,
    dataset_root: Optional[str] = "datasets",
    absolute_path: Optional[bool] = False,
    ...
    # if dataset.name == "darpa-cadets-tt-01" or "darpa-cadets-tt-01-exec-path" or "darpa-cadets-tt-exec-path-01-wo-default" or "darpa-cadets-tt-exec-path-01-wo-default-reduced": # TODO: make it more general
    if "darpa" in dataset.name:
        print(f"No anomalies should be loaded, they are already in the dataset.")
    else:
        dataset.load_anomalies(
            val_anom_type=val_anom_type,
