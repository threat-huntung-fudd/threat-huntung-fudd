...
class Evaluator(object):
    ...
    def __init__(self, anom_label: int = 0, classification=False):
        """
        Args:
        ...
        # self.valid_metric_list = ["accuracy_score"]
    ...
    def _eval_auc_and_ap_and_recall_at_k(self, y_pred, y_label):
        ...
        if len(np.unique(y_label)) <= 1:
            ...
            cl_report = classification_report(
                y_true=y_label, y_pred=y_pred_binary, output_dict=True
            )
        else:
            auc_score = roc_auc_score(y_score=y_pred, y_true=y_label)
            # additional f-1 score
            y_pred_binary = (y_pred > 0.5).astype(int)
            f1 = f1_score(y_true=y_label, y_pred=y_pred_binary)
            cl_report = classification_report(
                y_true=y_label, y_pred=y_pred_binary, output_dict=True
            )
            print(f"f1 score: {f1}")
    ...
    def eval(self, input_dict: dict) -> dict:
        ...
        if self.classification:
            y_pred, y_label = self._parse_and_check_input(input_dict)
            y_pred_class = np.argmax(y_pred, axis=1)
            accuracy = accuracy_score(y_true=y_label, y_pred=y_pred_class)
            precision = precision_score(
                y_true=y_label, y_pred=y_pred_class, average="weighted"
            )
            recall = recall_score(
                y_true=y_label, y_pred=y_pred_class, average="weighted"
            )
            f1 = f1_score(y_true=y_label, y_pred=y_pred_class, average="weighted")
            perf_dict = {
                "accuracy": accuracy,
                "precision": precision,
                "recall": recall,
            ...
            assert all(
                [
                    metric in self.valid_metric_list
                    for metric in input_dict["eval_metric"]
                ]
...
if __name__ == "__main__":
    ...
    input_dict = {
        ...
        "eval_metric": ["auc", "ap", "recall@k", "f1", "classification_report"],
    }
