...
class LinkAnomDetDataset(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.darpa_dataset = True if (self.name[:5] == "darpa") else False

        print(
            f"Custom dataset: {self.custom_dataset}, DARPA dataset: {self.darpa_dataset}"
        )

        if self.custom_dataset:
            if self.darpa_dataset:
    ...
    def generate_processed_files(self) -> pd.DataFrame:
        ...
        if (osp.exists(OUT_DF)) and (self.version_passed is True):
            ...
            elif self.name in ["synthetic", "lanl", "darpa-theia"]:
                df, edge_feat, node_ids = load_preprocessed_data(
                    self.meta_dict["fname"], self.meta_dict["fname_edge"], shift=False
                )
            # elif self.name in ["darpa-cadets-tt-01", "darpa-cadets-tt-01-exec-path", "darpa-cadets-tt-exec-path-01-wo-default", "darpa-cadets-tt-exec-path-01-wo-default_reduced"]:    #TODO: make it more general
            elif "darpa-cadets" in self.name:
                df, edge_feat, node_ids = csv_to_pd_data_darpa(self.meta_dict["fname"])
            elif "darpa-trace" in self.name:
                df, edge_feat, node_ids = csv_to_pd_data_darpa(self.meta_dict["fname"])
    ...
    def pre_process(self):
        ...
        # if self.name in ["darpa-cadets-tt-01", "darpa-cadets-tt-01-exec-path", "darpa-cadets-tt-exec-path-01-wo-default", "darpa-cadets-tt-exec-path-01-wo-default_reduced"]: #TODO: make it more general
        if "darpa-cadets" in self.name:
            edge_label = np.array(df["l"])  # eventlist is labeled so we can use it.
        elif "darpa-trace" in self.name:
            edge_label = np.array(df["l"])  # eventlist is labeled so we can use it.
        else:
