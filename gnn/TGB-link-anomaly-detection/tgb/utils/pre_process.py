...
"""
functions for darpa dataset
-------------------------------------------
"""

def csv_to_pd_data_darpa(fname: str) -> pd.DataFrame:
    """
    loading darpa dataset into pandas dataframe
    Parameters:
        fname: str, name of the input file
    Returns:
        df: a pandas dataframe containing the edgelist data
    ...
    idx = np.arange(t.shape[0])
    w = np.ones(t.shape[0])
    print(f"Unique sources: {len(np.unique(src))}, Unique destinations: {len(np.unique(dst))}")
    print(f"Unique timestamps: {len(np.unique(t))}, Unique labels: {len(np.unique(labels))}")

    return pd.DataFrame({"u": src, "i": dst, "ts": t, "idx": idx, "w": w, "l": labels}), msg, None
