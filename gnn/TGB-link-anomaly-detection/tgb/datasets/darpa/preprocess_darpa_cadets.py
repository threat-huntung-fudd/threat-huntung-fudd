...
class DatasetPaths:
    def __init__(self, root_folder, dataset_name):
        ...
        self.INPUT_FILE = os.path.join(self.ROOT_FOLDER, f"{dataset_name}.csv")
...
def preprocess(paths: DatasetPaths):
    ...
    """
    header = ["source_node","destination_node","timestamp","edge_label","edge_feature_1", "edge_feature_2", "edge_feature_3", "edge_feature_4"]
    df_graph = pd.read_csv(paths.INPUT_FILE, header=None, names=header, skiprows=1)

    print("Data read.")
    df_graph = df_graph.sort_values(by="timestamp")

    # src, dst, msg, t = map(np.array, [graph.src, graph.dst, graph.msg, graph.t])
    src = df_graph["source_node"].to_numpy()
    dst = df_graph["destination_node"].to_numpy()
    msg = df_graph["msg"].to_numpy()
    t = df_graph["timestamp"].to_numpy()
    ...
    # Split the data into benign and anomalous edges.
    anom_mask = np.concatenate([np.zeros(len(df_graph)//2, dtype=bool), np.ones(len(df_graph)//2, dtype=bool)]) # create anom mask based on 50/50 
    ...
    save_anomalies_for_tgb(
        file_name=paths.VAL_ANOM_FILE,  # Save anomalies for validation set.
        t=anom_t[val_mask],
...
if __name__ == "__main__":
    CADETS_PATHS = DatasetPaths(
        "/home/buchta/sambashare/gnn_darpa_lab/TGB-link-anomaly-detection-baselines/data/darpa_cadets_tt/", "darpa-tt-cadets-01"
    )
    preprocess(paths=CADETS_PATHS)
